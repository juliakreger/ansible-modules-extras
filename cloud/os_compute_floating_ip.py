...
def _get_floating_ip_id(module, neutron):
    ...
    try:
        ...
        module.fail_json(msg = "error in fetching the floating IPs: %s" % e.message)
...
def main():
    ...
    try:
        ...
        fip_state, floating_ip_id = _get_floating_ip_id(module, neutron)
        if module.params['state'] == 'present':
            server_info, server_obj = _get_server_state(module, nova)
            if not server_info:
                module.fail_json(msg = "The instance name provided cannot be found")

            server_pub_ip = cloud.get_server_public_ip(server_obj)

            if fip_state == 'attached':
                if module.params['ip_address'] == server_pub_ip:
                    module.exit_json(changed=False, result='attached', public_ip=module.params['ip_address'])
                else:
                    module.fail_json(msg="The IP address is attached to another instance")
